...
public class DatastoreServer
{
	...
	{
		// Need to implement
		String receivedCommand = StreamUtil.readLine(inputStream);
		if ("read".equalsIgnoreCase(receivedCommand)) {
			ReadCommand commandClass = new ReadCommand();
			return commandClass;
		}
		else if ("write".equalsIgnoreCase(receivedCommand)) {
			WriteCommand commandClass = new WriteCommand();
			return commandClass;
		}
		else if ("delete".equalsIgnoreCase(receivedCommand)) {
			DeleteCommand commandClass = new DeleteCommand();
			return commandClass;
		}
		else if ("directory".equalsIgnoreCase(receivedCommand)) {
			DirectoryCommand commandClass = new DirectoryCommand();
			return commandClass;
		}
		else {
			throw new ServerException("Unknown Request: " + receivedCommand);
		}
	}
